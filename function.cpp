...
std::shared_ptr<t_token> function_manager::select_realname_to_t_token(std::string function_name, std::string realname) {
    std::shared_ptr<t_token> ret = my_variable_manager->get_physicalname_from_table(convert_name_to_local(function_name, realname));
    return ret;
}
...
std::vector<std::string> function_manager::select_functionname_to_argument_physicalname_list(std::string funciton_name) {
    std::shared_ptr<function_info> func_info = get_function_info(funciton_name);
    std::vector<std::string> ret;
    int current_arg_cnt = 0;
    for(std::shared_ptr<t_token> arg : func_info->argument_table){
...
std::vector<std::shared_ptr<t_token>> function_manager::select_functionname_to_argument_t_token_info_list(std::string funciton_name) {
    std::shared_ptr<function_info> func_info = get_function_info(funciton_name);
...
void function_manager::set_function_info(std::string & func_name, std::vector<std::shared_ptr<t_token>> & input_args, std::vector<std::shared_ptr<t_token>> & retrn_vals, bool is_prototype){
    ...
    if(function_symbol_tbl.count(func_name) == 0) {
        std::shared_ptr<function_info> temp (new function_info());
        // 関数名を保存
        ...
        for(std::shared_ptr<t_token> y : retrn_vals){
    ...
    if( !is_prototype ){    // プロトタイプ宣言では変数の実体は作らない
        // 引数の実体を登録する
        for(std::shared_ptr<t_token> x : input_args){
...
std::vector<std::string> function_manager::select_functionname_to_returnval_physicalname_list(std::string funciton_name){
    ...
    std::vector<std::string> ret;
    int current_arg_cnt = 0;
    for(std::shared_ptr<t_token> arg : func_info->return_val_table){
...
std::vector<std::shared_ptr<t_token>> function_manager::select_functionname_to_returnval_t_token_info_list(std::string funciton_name) {
    std::shared_ptr<function_info> func_info = get_function_info(funciton_name);
...
void function_manager::set_localname_and_realname(std::string func_name, std::string realname, std::shared_ptr<t_token> token, E_VARIABLE_KIND variable_kind) {
    std::shared_ptr<function_info> func_info = get_function_info(func_name);
    std::shared_ptr<t_token> temp( new t_token(*token) );
    if(!func_info) {
...
// 関数名から関数情報を引く
std::shared_ptr<function_info> function_manager::get_function_info(std::string & func_name) {
    if(function_symbol_tbl.count(func_name) != 0) {
...
void function_manager::set_localname_to_conv_tbl(std::string function_name, std::string realname, std::string localname) {
    std::shared_ptr<function_info> func = get_function_info(function_name);
